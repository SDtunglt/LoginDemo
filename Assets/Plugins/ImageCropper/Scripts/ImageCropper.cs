...
public class ImageCropper : MonoBehaviour
{
	...
	public Texture2D inheritedImage;
...
#pragma warning restore 0649
	...
	public Texture2D CropSelection( int width, int height )
	{
		...
		// Debug.Log(("inheritedImage size: " + inheritedImage.width + " " + inheritedImage.height));
		int _x = (int) ((selectionCenter.x - selectionSize.x / 2) / m_orientedImageSize.x * m_originalImageSize.x);
		int _y = (int) ((selectionCenter.y - selectionSize.y / 2) / m_orientedImageSize.y * m_originalImageSize.y);
		
		Color[] pix = inheritedImage.GetPixels(_x, _y, width, height);
		...
		// 	
		// 	SDLogger.Log("Cropped image size: " + width + " " + height);
