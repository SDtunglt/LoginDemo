	...
	public class LeanGameObjectPool : MonoBehaviour, ISerializationCallbackReceiver
	{
		...
		public bool TrySpawn(Vector3 position, Quaternion rotation, Transform parent, bool worldPositionStays, ref GameObject clone)
		{
			if (prefab != null)
			{
				...
				for (var i = despawnedClones.Count - 1; i >= 0; i--)
				{
					...
					if (Warnings == true) SDLogger.LogWarning("This pool contained a null despawned clone, did you accidentally destroy it?", this);
				}
			...
			{
				if (Warnings == true) SDLogger.LogWarning("You're attempting to spawn from a pool with a null prefab", this);
			}
		...
		private bool TryDespawnOldest(ref GameObject clone, bool registerDespawned)
		{
			...
			while (spawnedClonesList.Count > 0)
			{
				...
				if (Warnings == true) SDLogger.LogWarning("This pool contained a null spawned clone, did you accidentally destroy it?", this);
			}
		...
		public void Despawn(GameObject clone, float t = 0.0f)
		{
			...
			{
				if (Warnings == true) SDLogger.LogWarning("You're attempting to despawn a null gameObject", this);
			}
		}
		...
		public void PreloadOneMore()
		{
			if (prefab != null)
			{
				...
				if (Warnings == true && Capacity > 0 && Total > Capacity) SDLogger.LogWarning("You've preloaded more than the pool capacity, please verify you're preloading the intended amount.", this);
			}
			else
			{
				if (Warnings == true) SDLogger.LogWarning("Attempting to preload a null prefab.", this);
			}
		}
		...
		protected virtual void Update()
		{
			...
			for (var i = delays.Count - 1; i >= 0; i--)
			{
				...
				{
					if (Warnings == true) SDLogger.LogWarning("Attempting to update the delayed destruction of a prefab clone that no longer exists, did you accidentally delete it?", this);
				}
			}
		}
		...
		private void RegisterPrefab()
		{
			if (prefab != null)
			{
				...
				{
					SDLogger.LogWarning("You have multiple pools managing the same prefab (" + prefab.name + ").", existingPool);
				}
				else
				{
		...
		private void TryDespawn(GameObject clone)
		{
			...
			{
				if (Warnings == true) SDLogger.LogWarning("You're attempting to despawn a GameObject that wasn't spawned from this pool, make sure your Spawn and Despawn calls match.", clone);
			}
		}
