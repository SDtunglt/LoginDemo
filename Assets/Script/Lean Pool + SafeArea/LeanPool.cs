			...
			if (prefab != null)
			{
				...
				if (pool.TrySpawn(position, rotation, parent, worldPositionStays, ref clone) == true)
				{
					...
					if (Links.Remove(clone) == true)
					{
						...
						{
							SDLogger.LogWarning("You're attempting to spawn a clone that hasn't been despawned. Make sure all your Spawn and Despawn calls match, you shouldn't be manually destroying them!", clone);
						}
					}
		...
		public static void Despawn(GameObject clone, float delay = 0.0f)
		{
			if (clone != null)
			{
				...
				else
				{
					...
					{
						SDLogger.LogWarning("You're attempting to despawn a gameObject that wasn't spawned from this pool", clone);
			...
			{
				SDLogger.LogWarning("You're attempting to despawn a null gameObject", clone);
			}
		}
	}
}